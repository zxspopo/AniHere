...
if ($url[$num]=="err") {
	echo "<script> $(function(){ $.jBox.tip('登录失败！学号或密码错误！', 'error');}); </script>";}
if ($url[$num]=="login") {
...
//学校教务在线验证相关
if ($url[$num]=="serr") {
	echo "<script> $(function(){ $.jBox.tip('对不起！您的学号或者密码不匹配！', 'error');}); </script>";}	
//注册相关
if ($url[$num]=="reg") {
	echo "<script> $(function(){ $.jBox.tip('恭喜你！注册成功！请您先登录！', 'success');}); </script>";}
if ($url[$num]=="regged") {
	echo "<script> $(function(){ $.jBox.tip('对不起！您已经是注册会员了！', 'error');}); </script>";}
if ($url[$num]=="resid") {
	echo "<script> $(function(){ $.jBox.tip('对不起！这个学号已经被注册过！', 'error');}); </script>";}
if ($url[$num]=="regwrong") {
