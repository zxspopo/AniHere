...
<div id="bdshare" class="bdshare_t bds_tools get-codes-bdshare" data="{'pic':'<?php if ($video->imageUrl==""){ echo '/images/noimage.jpg';}else{echo $video->imageUrl;} ?>','text':'<?=$video->url?> 我在AniHere! 看到一部不错的动画： <?=$video->title?>  '}">
<a class="bds_tsina"></a>
...
<!-- Baidu Button END --></div></div>
        <div id"des" style="margin-top: 20px; padding-bottom:10px;color: #666;text-indent :0px;">
		 <? if($video->description==""){ ?> 这个ANIer很懒，什么也没留下 <? }else{ echo nl2br($video->description); } ?>
	</div>
      </div>
      <!-- /观看区域-->
	...
	<div class="ds-thread" data-thread-key="video<?=$video->id?>" data-title="<?=$video->title?>" data-category="<?=$tag->name?>" data-author-key="<?=$video->userid?>"></div>
...
<script type="text/javascript">
    ...
    var wumiiSitePrefix = "http://anihere.com/";
